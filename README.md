...
- Public NFT ownership checks now run client-side with `getOwnedObjects` plus the Kiosk SDK. When `NEXT_PUBLIC_SUI_RPC_URL` points at a browser-safe Tatum gateway, the public NFT gate prefers that endpoint over the default fullnode path so static production builds can avoid browser CORS failures on `fullnode.mainnet.sui.io`.
- `VITE_NFT_OWNERSHIP_API_URL` and [`api/nft-ownership-check.ts`](./api/nft-ownership-check.ts) remain available only as optional server-side helpers for local diagnostics or future deployments; they are no longer required for the public NFT gate path.
