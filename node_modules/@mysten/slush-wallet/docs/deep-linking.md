...
> **Note:** For most integrations, use `my.slush.app` HTTPS links. They work on iOS, Android, and
> desktop browsers, opening the native app when installed or falling back to the web app.

> **Warning:** Deep linking is not currently supported for the Slush browser extension.

> **Warning:** Always URL-encode parameter values, especially token type addresses which contain
> special characters like colons (`:`). See [URL Encode Parameters](#url-encode-parameters) for
> examples.
...
> **Note:** Unlike custom URL schemes (`slush://`), `my.slush.app` links never fail - they always
> open either the native app or the web app, ensuring users can complete their intended action.
...
> Use the `@mysten/payment-kit` SDK to generate properly formatted payment URIs. See the
> [Payment Kit documentation](/payment-kit/getting-started) for more details.
