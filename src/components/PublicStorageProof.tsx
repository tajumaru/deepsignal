...
import { PublicSignalMetaChip } from "./PublicSignalMeta";

interface PublicStorageProofProps {
  blobId?: string | null;
...
function getVerificationLabel(status: WalrusVerificationStatus, t: ReturnType<typeof useI18n>["t"]) {
  switch (status) {
    case "verifying":
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="storage-proof-icon">
      <path
        ...
        strokeWidth="1.8"
      />
      <path d="M13 4h7v7" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
      <path
        d="m11 13 8.5-8.5"
        fill="none"
        stroke="currentColor"
...
function VerifyIcon({ status }: { status: WalrusVerificationStatus }) {
  if (status === "verifying") {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true" className="storage-proof-icon storage-proof-icon-spin">
        ...
        <path d="M20 12a8 8 0 0 0-8-8" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="2" />
      </svg>
    );
  }
  if (status === "verified") {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true" className="storage-proof-icon">
        <path d="M12 3.5 19 6v5.25c0 4.35-2.95 7.45-7 9.25-4.05-1.8-7-4.9-7-9.25V6l7-2.5Z" fill="none" stroke="currentColor" strokeLinejoin="round" strokeWidth="1.8" />
        <path d="m8.75 12.1 2.1 2.1 4.55-4.7" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
      </svg>
    );
  }
  if (status === "not-found" || status === "failed") {
    return (
      <svg viewBox="0 0 24 24" aria-hidden="true" className="storage-proof-icon">
        <path d="M12 3.5 19 6v5.25c0 4.35-2.95 7.45-7 9.25-4.05-1.8-7-4.9-7-9.25V6l7-2.5Z" fill="none" stroke="currentColor" strokeLinejoin="round" strokeWidth="1.8" />
        <path d="m9.4 9.4 5.2 5.2M14.6 9.4l-5.2 5.2" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" />
      </svg>
    );
  }
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="storage-proof-icon">
      <path d="M12 3.5 19 6v5.25c0 4.35-2.95 7.45-7 9.25-4.05-1.8-7-4.9-7-9.25V6l7-2.5Z" fill="none" stroke="currentColor" strokeLinejoin="round" strokeWidth="1.8" />
      <path d="M12 8v4.25l2.6 1.55" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
    </svg>
  );
}

export function PublicStorageProof({
  blobId,
  proof,
  label = "Storage proof",
  fallbackSize,
  compact = false,
}: PublicStorageProofProps) {
  const { t } = useI18n();
  ...
  const isWalrusBlob = Boolean(resolvedBlobId && !isPublicLocalFallbackBlob(resolvedBlobId));
    ...
    <section className={`storage-proof ${compact ? "storage-proof-compact" : ""}`.trim()}>
      <div className="storage-proof-header">
        <div className="storage-proof-title">
          <span>{label}</span>
          <PublicSignalMetaChip type="blob" value={resolvedBlobId} />
        </div>
